...
impl<'a> Jbig2Encoder<'a> {
    ...
    pub fn flush(&mut self) -> Result<Vec<u8>> {
        ...
        if self.global_symbols.len() > 300 {
            ...
            let kept_indices: FxHashSet<usize> = symbol_indices_with_usage
                .into_iter()
                .take(300)
                .map(|(i, _)| i)
                .collect();
        ...
        let page_local_symbols: Vec<Vec<usize>> = self
            ...
            .map(|(page_num, _)| {
                self.global_symbols
                    ...
                    .enumerate()
                    .filter(|(i, _)| {
                        self.symbol_usage[*i] == 1 && self.symbol_pages[*i].contains(&page_num)
                    })
                    .map(|(i, _)| i)
                    .collect()
            })
...
pub fn build_dictionary_and_get_instances(
    ...
    for (rect, symbol) in symbols.iter() {
        ...
        let max_err = ((symbol.image.width * symbol.image.height) / 10).max(3) as u32;
