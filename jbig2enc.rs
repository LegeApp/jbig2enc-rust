    ...
    pub fn flush(&mut self) -> Result<Vec<u8>> {
        ...
        for (page_num, page) in self.pages.iter().enumerate() {
            ...
            if self.config.symbol_mode {
                ...
                let at_pixels: &[(i8, i8)] = &[(-1, -1), (0, -1), (1, -1), (-1, 0)];
                let coder_data = Jbig2ArithCoder::encode_generic_payload(&page.image, 0, at_pixels)?;
                ...
                // Write adaptive template pixels
                for (dx, dy) in at_pixels.iter() {
                    generic_region_payload.push(*dx as u8);
                    generic_region_payload.push(*dy as u8);
                }
