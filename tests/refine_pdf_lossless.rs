...
fn jbig2dec() -> PathBuf {
    std::env::var("JBIG2DEC")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("jbig2dec"))
}
...
fn pdf_refinement_lossless_roundtrip() {
    ...
    eprintln!(
        ...
        m.symbol_stats.symbols_discovered,
        m.symbol_stats.symbols_exported,
        m.symbol_stats.refined_hits,
        px.len()
    );
    assert_eq!(
        diffs, 0,
        "PDF symbol-mode roundtrip not lossless: {diffs} px differ"
    );
}
