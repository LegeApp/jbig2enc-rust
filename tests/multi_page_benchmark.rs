...
//!   BENCH_PAGES   — comma-separated page counts (default: "10,50")
//!                   example: BENCH_PAGES=1,5,10,20,50,100,all
//!   BENCH_WRITE   — set to "1" to write PDFs to disk (default: memory only)

use jbig2enc_rust::jbig2enc::{Jbig2Encoder, PdfSplitOutput};
...
fn load_pbm_to_array(path: &Path) -> (ndarray::Array2<u8>, u32, u32) {
    let file = std::fs::File::open(path).unwrap_or_else(|e| panic!("Failed to open PBM {}: {}", path.display(), e));
    ...
    let (width, height) = loop {
        ...
        let dims: Vec<usize> = trimmed.split_whitespace()
...
fn raw_jbig2_stats(split: &PdfSplitOutput) -> (usize, usize, Vec<f64>) {
    ...
    let page_sizes_kb: Vec<f64> = split.page_streams.iter()
...
fn parse_page_counts(total_pages: usize) -> Vec<usize> {
    if let Ok(val) = std::env::var("BENCH_PAGES") {
        let mut counts: Vec<usize> = val.split(',')
            .map(|s| s.trim())
            .map(|s| {
                if s.eq_ignore_ascii_case("all") {
                    total_pages
                } else {
                    s.parse().unwrap_or_else(|_| panic!("Invalid BENCH_PAGES value: {s}"))
                }
            })
            .collect();
...
fn configs_for_count(count: usize, total_pixels: u64) -> Vec<(&'static str, Jbig2Config)> {
    ...
    // auto_thresh is O(n²) on symbols — skip for large datasets
    let allow_auto_thresh = count <= 10 && total_pixels <= 150_000_000;
    if allow_auto_thresh {
...
fn multi_page_compression_benchmark() {
    ...
    assert!(total_pages >= 10, "Need at least 10 PBM pages, found {}", total_pages);
    ...
    let all_pages: Vec<PageInfo> = pbm_files
        ...
        .map(|p| {
            ...
            PageInfo { array, width: w, height: h, pbm_bytes }
        })
        .collect();
    ...
    // ── Table header ─────────────────────────────────────────────
    println!("{:<6} {:<10} {:>10} {:>10} {:>10} {:>8} {:>10} {:>8} {:>8} {:>8}",
        "Pages", "Mode", "Raw KB", "Globals", "PDF KB", "vs Gen",
        "Enc (s)", "ms/pg", "MPix/s", "vs PBM");
    println!("{}", "─".repeat(102));
    ...
    for &count in &page_counts {
        ...
        let total_pixels: u64 = pages.iter()
        ...
        assert_eq!(gen_result.split.page_streams.len(), pages.len(), "generic: wrong page stream count");
        ...
        let gen_pdf_bytes = build_pdf_size(
            &gen_result.split,
            pages,
            if write_pdfs { Some(&gen_pdf_path) } else { None },
        );
        ...
        println!("{:<6} {:<10} {:>9.1} {:>9.1} {:>9.1} {:>7}  {:>9.2} {:>7.1} {:>7.1} {:>7.1}%",
            count, "generic",
        ...
        for (label, cfg) in &configs {
            ...
            assert_eq!(result.split.page_streams.len(), pages.len(),
            ...
            let pdf_bytes = build_pdf_size(
                &result.split,
                pages,
                if write_pdfs { Some(&sym_pdf_path) } else { None },
            );
            ...
            println!("{:<6} {:<10} {:>9.1} {:>9.1} {:>9.1} {:>6.1}%  {:>9.2} {:>7.1} {:>7.1} {:>7.1}%",
                count, label,
    ...
    for row in &all_rows {
        if row.mode != "generic" {
            println!("  {} {}p: globals {:.1}KB, pages avg {:.1}KB min {:.1}KB max {:.1}KB",
                row.mode, row.pages,
                row.globals_bytes as f64 / 1024.0,
                row.avg_page_kb, row.min_page_kb, row.max_page_kb);
        }
    }
    println!();
    ...
    if write_pdfs {
        ...
        let mut csv = String::from("pages,mode,encode_secs,ms_per_page,mpix_per_s,raw_jbig2_bytes,globals_bytes,pdf_bytes,savings_vs_generic,savings_vs_pbm\n");
        for row in &all_rows {
            csv += &format!("{},{},{:.4},{:.2},{:.2},{},{},{},{:.2},{:.2}\n",
                row.pages, row.mode, row.encode_secs, row.ms_per_page,
                row.mpix_per_s, row.raw_jbig2_bytes, row.globals_bytes,
                row.pdf_bytes, row.savings_vs_generic, row.savings_vs_pbm);
        }
